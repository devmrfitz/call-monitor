    ...
    async getToken(username, password) {
        try {
            const response = await axios_1.default
                .create({
                baseURL: `${this.apiUrl}`,
                headers: { Authorization: `Basic ${(0, btoa_1.default)(username + ":" + password)}` }
            })
                .get("/token");
            return response.data.data;
        }
        catch (err) {
            (0, routr_errors_1.default)(err);
        }
    }
    async list(params, accessKeyId) {
        const queryParams = (p) => Object.keys(p).map((k) => `${k}=${p[k]}`);
        try {
            const url = `${this.apiUrl}/${this.resource}?token=${this.token}&filter=@.metadata.accessKeyId=='${accessKeyId}'&${queryParams(params).join("&")}`;
